...
const s = {
  ...
  main: { flex: 1, overflowY: "auto" as const, padding: 32 } as CSSProperties,
            ...
            return (
              ...
              >
                ...
                <div style={{ textAlign: "right" }}>
                  ...
                  <span style={s.badge(statusColor ?? C.textSub)}>{item.status}</span>
                </div>
              </div>
            );
          })}
...
function BdsSimulator() {
  ...
  const bidLimit =
    (Number(form.yahooPrice) || 0) - yahooFee - misc - repair - profit;
  const save = () => {
  ...
  return (
    <div>
      ...
      <div style={s.card}>
        ...
        <button
          style={{ ...s.btn("primary"), marginTop: 12, width: "100%" }}
          onClick={save}
        >
          保存する
        </button>
            ...
            <tbody>
              {history.map((h: Record<string, unknown>) => (
                <tr key={String(h.id)}>
                  ...
                  <td style={s.td}>
                    {String(h.maker)} {String(h.model)}{" "}
                    {h.year ? `(${h.year})` : ""}
                  </td>
                  <td style={s.td}>
                    <span style={s.badge(C.blue)}>{String(h.condition)}</span>
                  </td>
                  <td style={s.td}>{fmt(Number(h.yahooPrice))}</td>
                  <td
                    style={{
                      ...s.td,
                      color: C.orange,
                  ...
                  >
                    {fmt(
                      Number(h.bidLimit) > 0 ? Number(h.bidLimit) : 0
                    )}
                  </td>
                </tr>
              ))}
      ...
      <div style={s.grid2}>
        {[
          ...
          <div
            ...
            onClick={() => nav(id)}
          >
            <div style={{ fontSize: 32, marginBottom: 12, color: C.orange }}>
              {icon}
            </div>
    ...
    <div>
      ...
      <div style={s.grid2}>
        <div style={s.card}>
          ...
          {[
            { label: "宛先", key: "to", placeholder: "株式会社○○ 御中" },
            { label: "件名", key: "title" },
            { label: "見積日", key: "date" },
            { label: "有効期限（日）", key: "validDays" },
        ...
        <div style={s.card}>
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <select
              ...
              value={info.tax}
              onChange={(e) =>
                setInfo((p) => ({ ...p, tax: Number(e.target.value) }))
              }
            >
...
function Invoice() {
  ...
  const set = (k: keyof typeof form, v: string) =>
    setForm((p) => ({ ...p, [k]: v }));
  const addItem = () =>
    setItems((p) => [...p, { name: "", qty: 1, unitPrice: "" }]);
  const subtotal = items.reduce(
    (a, item) => a + (Number(item.qty) * Number(item.unitPrice) || 0),
    0
  );
    ...
    <div>
      ...
      <div style={s.grid2}>
        <div style={s.card}>
          ...
          {[
            { label: "宛先", key: "to", placeholder: "株式会社○○ 御中" },
            { label: "請求番号", key: "invoiceNo" },
            { label: "請求日", key: "date" },
            { label: "支払期限", key: "dueDate", placeholder: "例: 2026-03-31" },
          ].map(({ label, key, placeholder }) => (
            <div key={key} style={s.formRow}>
              <label style={s.label}>{label}</label>
              <input
    ...
    <div>
      ...
      <div style={s.grid2}>
        <div style={s.card}>
          ...
          <div style={s.formRow}>
            ...
            <input
              ...
              placeholder="例: Honda CB400SF 2019年式 実働 美品"
            />
          </div>
          <div style={s.grid2}>
            <div style={s.formRow}>
          ...
          <div style={s.grid2}>
            ...
            <div style={s.formRow}>
              <label style={s.label}>走行距離(km)</label>
              <input
                style={s.input}
                value={form.mileage}
          ...
          <div style={s.formRow}>
            <label style={s.label}>商品説明</label>
            <textarea
              style={{ ...s.input, height: 80, resize: "vertical" }}
              value={form.description}
              onChange={(e) => set("description", e.target.value)}
              placeholder="車両の状態・特徴を詳しく記載"
            />
          </div>
          <div style={s.grid2}>
            <div style={s.formRow}>
        ...
        <div style={{ ...s.card, background: "#0d0d0f" }}>
          <div style={{ ...s.cardTitle, color: C.orange }}>
            ヤフオク プレビュー
          </div>
          <div
            style={{
          ...
          >
            ...
            >
              {form.title ||
                `${form.maker} ${form.model} ${form.year ? form.year + "年式" : ""} ${form.condition}`.trim() ||
                "タイトルを入力してください"}
            </div>
            {form.mileage && (
              <div style={{ fontSize: 12, color: C.textSub }}>
                走行距離: {Number(form.mileage).toLocaleString()} km
              </div>
            )}
          </div>
          <div
          ...
          >
            ...
            {form.year && (
              <span style={s.badge(C.yellow)}>{form.year}年式</span>
            )}
          </div>
          {form.description && (
            <div
          ...
          >
            <span style={{ fontSize: 12, color: C.textMuted }}>開始価格</span>
            <span style={{ fontSize: 16, fontWeight: "bold" }}>
              {form.startPrice ? fmt(form.startPrice) : "—"}
            </span>
          </div>
          {form.buyNowPrice && (
            <div
...
type InventoryItem = (typeof SAMPLE_INVENTORY)[number] & { id: number };
...
function Inventory() {
  ...
  const setN = (k: string, v: string | number) =>
    setNewItem((p) => ({ ...p, [k]: v }));
  const filtered =
    filter === "全て"
      ? inventory
  ...
  const formFields = [
    ...
    { label: "車種", key: "model", type: "input" as const, placeholder: "例: CB400SF" },
    { label: "年式", key: "year", type: "input" as const, placeholder: "2019" },
    {
      label: "状態",
      key: "condition",
      type: "select" as const,
    ...
    { label: "仕入価格", key: "purchasePrice", type: "number" as const, placeholder: "0" },
    { label: "販売予定価格", key: "sellingPrice", type: "number" as const, placeholder: "0" },
    {
      label: "ステータス",
      key: "status",
      type: "select" as const,
    ...
    { label: "仕入日", key: "purchased", type: "input" as const },
  ];
  return (
    <div>
      ...
      >
        <div style={{ display: "flex", gap: 6 }}>
          {["全て", "在庫", "出品中", "整備中", "売約済"].map((st) => (
            <button
              key={st}
              style={{
                ...s.btn(filter === st ? "primary" : "ghost"),
                padding: "6px 14px",
                fontSize: 12,
              }}
                ...
                <div key={field.key}>
                  ...
                  {field.type === "select" ? (
                    ...
                    <input
                      ...
                      value={String(newItem[field.key] ?? "")}
                      onChange={(e) =>
                        setN(field.key, field.type === "number" ? Number(e.target.value) : e.target.value)
                      }
                      placeholder={placeholder}
                    />
                  )}
    ...
    <div>
      ...
      <div style={s.grid3}>
        <div style={s.kpiCard(C.blue)}>
          ...
          <div style={{ ...s.kpiValue, fontSize: 20 }}>
            {fmt(totalForecast)}
          </div>
        </div>
        <div style={s.kpiCard(C.orange)}>
        ...
        <div
          style={s.kpiCard(achieveRate >= 100 ? C.green : C.yellow)}
        >
...
function Settings() {
  ...
  const set = (k: keyof typeof cfg, v: string | number) =>
    setCfg((p) => ({ ...p, [k]: v }));
  return (
    <div>
        ...
        <div style={s.grid2}>
          {[
            ...
            <div key={key} style={s.formRow}>
              ...
              <input
                style={s.input}
                value={String(cfg[key as keyof typeof cfg])}
  ...
  const sections = [
    {
      title: "ダッシュボード",
      content:
        "在庫のKPI・月次推移を一覧表示。仕入れ総額・想定売上・粗利を即把握できます。",
    },
    {
      title: "BDS入札シミュレーター",
      content:
    ...
    },
    {
      title: "BDS過去相場",
      content:
        "車種ごとの落札価格を集計・表示。平均・最低・最高価格とトレンドで仕入れ判断の参考に。",
    },
    {
      title: "見積書・請求書",
      content:
        "品目・数量・単価を入力して書類作成。消費税率の選択、印刷機能対応。",
    },
    {
      title: "在庫管理",
      content:
        "仕入れた車両を登録・管理。ステータス別フィルタリング可能。想定利益も自動計算。",
    },
    {
      title: "予想 vs 実績",
      content:
        "月次の売上予測と実績を比較。達成率・粗利推移をグラフで可視化。",
    },
  ];
  return (
    <div>
            ...
            <div key={group.group}>
              ...
              {group.items.map((item) => (
                <div
                  ...
                  style={s.navItem(
                    page === item.id ||
                      (page === "quote" && item.id === "documents") ||
                      (page === "invoice" && item.id === "documents")
                  )}
